...
arg.add_argument('-embed_dim', type=int, default=150, help='word embedding size (100, 150, 200)')
...
prediction_dir = '../output/prediction/LineBB/'
...
def train_model(dataset_name):
    ...
    train_code3d, train_label, train_word_edge, tarin_line_edge, train_block_size, train_block_neighbors = get_code3d_and_label(train_df, True)
    valid_code3d, valid_label, valid_word_edge, valid_line_edge, valid_block_size, valid_block_neighbors = get_code3d_and_label(valid_df, True)

    train_code3d_line, train_file_label, train_line_label = get_code3d_line_and_label(train_df, True, max_train_LOC)
    valid_code3d_line, valid_file_label, valid_line_label = get_code3d_line_and_label(valid_df, True, max_train_LOC)
    ...
    x_train_line_vec = get_x_vec(train_code3d_line, word2vec)
    x_valid_line_vec = get_x_vec(valid_code3d_line, word2vec)

    max_sent_len = min(max([len(sent) for sent in (x_train_vec)]), max_train_LOC)
    max_sent_line_len = min(max([len(sent) for sent in (x_train_line_vec)]), max_train_LOC)
    ...
    tarin_code_line_vec_pad = torch.tensor(pad_code(x_train_line_vec, max_sent_line_len))
    ...
    valid_code_line_vec_pad = torch.tensor(pad_code(x_valid_line_vec, max_sent_line_len))
    ...
    if total_checkpoints == 0:
        ...
        checkpoint_nums = [int(re.findall(r'\d+', s)[0]) for s in checkpoint_files]
    ...
    for epoch in tqdm(range(current_checkpoint_num, num_epochs+1)):
        ...
        for inputs, inputs_line, labels, word_edge_index, line_edge_index, basic_block_size, block_neighbors in batch_generator(tarin_code_vec_pad, tarin_code_line_vec_pad, train_label_tensor,
                                                                                train_padded_edge, tarin_line_edge, train_block_size, train_block_neighbors, batch_size):

            output, _, __, ___ = model(inputs, inputs_line, word_edge_index, line_edge_index, basic_block_size, block_neighbors)
        ...
        with torch.no_grad():
            ...
            for inputs, inputs_line, labels, word_edge_index, line_edge_index, basic_block_size, block_neighbors in batch_generator(valid_code_vec_pad,
                                                                                    valid_code_line_vec_pad,
                                                                                    valid_label_tensor,
                                                                                    valid_padded_edge,
                                                                                    valid_line_edge, valid_block_size, valid_block_neighbors, batch_size):
...
dataset_name = args.dataset # 修改数据集项目
